...
const Centro = styled.div`
  ...
  .status {
    ...
    letter-spacing: 3px;
    color: ${({ $rodando }) =>
      $rodando ? '#a5ef1c ' : 'rgba(255,255,255,0.25)'};
    text-transform: uppercase;
...
const Ponto = styled.span`
  ...
  background: ${({ $ativo, theme }) =>
    $ativo ? theme.cores.primaria : 'rgba(255,255,255,0.2)'};
  box-shadow: ${({ $ativo }) =>
    $ativo ? '0 0 10px rgba(165, 239, 28, 0.5)' : 'none'};
  animation: ${({ $ativo }) => ($ativo ? pulsarPonto : 'none')} 1.2s ease-in-out
    infinite;
`;
