...
const Crono = styled.span`
  ...
  color: ${({ theme, $r }) =>
    $r ? theme.cores.primaria : theme.cores.textoSuave};
...
const BarraCor = styled.span`
  ...
  border-radius: 2px;
  background: ${({ $c }) => $c};
  box-shadow: 0 0 8px ${({ $c }) => hexParaRgba($c, 0.45)};
`;
...
const GolNum = styled.span`
  ...
  color: ${({ theme }) => theme.cores.texto};
  ${({ $a }) =>
    $a && `animation:${glisseScore} .3s cubic-bezier(.34,1.56,.64,1);`}
`;
...
const CartaoTraco = styled.span`
  ...
  box-shadow: 0 0 5px
    ${({ $c }) =>
      $c === 'amarelo' ? 'rgba(234,179,8,.45)' : 'rgba(220,38,38,.45)'};
`;
      ...
      <Barra>
        ...
        <Shell>
          ...
          <CartoesBar>
            <CartoesMetade>
              {tCasa.map((c, i) => (
                <CartaoTraco key={`c${i}`} $c={c} />
              ))}
            </CartoesMetade>
            <CartoesMetade>
              {tVis.map((c, i) => (
                <CartaoTraco key={`v${i}`} $c={c} />
              ))}
            </CartoesMetade>
          </CartoesBar>
