...
const CorpoPlacar = styled.div`
  display: flex;
  align-items: stretch;
  background: linear-gradient(180deg, #47124f 0%, ${PL_ROXO} 100%);
  border-radius: 0 0 12px 12px;
  overflow: visible;
  animation: ${({ $penaltis }) => ($penaltis ? transicaoModo : 'none')} 0.5s
    ease;
`;
...
const BlocoTime = styled.div`
  ...
  border-bottom: 4px solid ${({ $cor }) => $cor};
  min-width: 150px;
...
const StatusBar = styled.div`
  ...
  .label {
    ...
    letter-spacing: 3px;
    text-transform: uppercase;
    color: ${({ theme, $ativo }) => ($ativo ? PL_MINT : theme.cores.texto)};
  }
`;
      ...
      <BarraPlacar>
        <LinhaTempo>
          ...
          <ChipTempo>
            ...
            <ChipAcrescimo $visivel={acrescimo > 0}>
              <span>+{Math.max(0, acrescimo || 0)}:00</span>
            </ChipAcrescimo>
          </ChipTempo>
        </LinhaTempo>
