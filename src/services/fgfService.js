...
function extrairUltimaRodada(html, entradas, rodadaAlvo = 0) {
  ...
  const lerItem = (item) => {
    ...
    for (const bloco of item.querySelectorAll('.carousel-conteudo')) {
      ...
      const mSlug = (
        bloco.querySelector('.contra a')?.getAttribute('href') || ''
...
function lerCacheAR() {
  try {
    ...
    if (!bruto) return null;
    const cache = JSON.parse(bruto);
    if (!Array.isArray(cache?.jogadores) || !cache.jogadores.length)
      return null;
    return cache;
  } catch (e) {
    return null;
...
function extrairProximasRodadas(html, entradas, limite = 3) {
  ...
  const lerItem = (item) => {
    ...
    for (const bloco of item.querySelectorAll('.carousel-conteudo')) {
      ...
      if (/^\d+\s*[Xx×]\s*\d+$/.test(placarTexto)) realizados++;
  ...
  for (const r of lidas) {
    if (r.realizados > 0 && r.numero > ultimaJogada) ultimaJogada = r.numero;
  }

  const futuras = lidas
    .filter((r) => r.numero > ultimaJogada)
    .sort((a, b) => a.numero - b.numero);

  return futuras
    .slice(0, limite)
    .map((r) => ({ titulo: r.titulo, jogos: r.jogos }));
}

function lerCachePR() {
  try {
    const bruto = localStorage.getItem(CHAVE_CACHE_PR);
    if (!bruto) return null;
    const cache = JSON.parse(bruto);
    if (!Array.isArray(cache?.rodadas) || !cache.rodadas.length) return null;
    return cache;
  } catch (e) {
    return null;
  }
...
function gravarCachePR(rodadas) {
  try {
    localStorage.setItem(
      CHAVE_CACHE_PR,
      JSON.stringify({ quando: Date.now(), rodadas }),
    );
  } catch (e) {
    console.warn('FGF: falha ao salvar cache das Próxima Rodada.', e);
  }
}

export async function importarProximasRodadasFGF({
  forcar = false,
  limite = 1,
} = {}) {
  const cache = lerCachePR();
  const idade = cache ? Date.now() - (cache.quando || 0) : Infinity;
  if (!forcar && cache && idade < TTL_CACHE_MS) {
    return { rodadas: cache.rodadas, origem: 'cache' };
  }
  ...
  try {
    ...
    const dadosClass = extrairClassificacao(html);
    const entradas = mapaNomeParaSigla(dadosClass);
    const rodadas = extrairProximasRodadas(html, entradas, limite);
    if (!rodadas.length) {
      throw new Error('Próxima Rodada não encontradas na página da FGF');
    }
    gravarCachePR(rodadas);
    return { rodadas, origem: 'rede' };
  } catch (erro) {
    if (cache) {
      return { rodadas: cache.rodadas, origem: 'cache' };
    }
...
export async function importarUltimaRodadaFGF({
  forcar = false,
  rodadaAlvo = 0,
} = {}) {
