...
const estadoPadrao = {
  competicao: 'CAMPEONATO GAUCHO SÉRIE A2',
...
function carregar() {
  try {
    ...
    if (bruto) {
      ...
      if (!salvo.visitante || !Array.isArray(salvo.visitante.cobrancas))
        salvo.visitante = criarLado();
...
export function definirTexto(campo, valor) {
  setEstado((estado) => {
    if (campo === 'competicao')
      estado.competicao = String(valor).slice(0, 60).toUpperCase();
    else if (campo === 'fase')
      estado.fase = String(valor).slice(0, 40).toUpperCase();
    return estado;
  });
}
...
export function definirCobranca(ladoNome, indice, resultado) {
  setEstado((estado) => {
    ...
    lado.cobrancas[indice] =
      resultado === lado.cobrancas[indice] ? null : resultado;
    return estado;
  });
}
