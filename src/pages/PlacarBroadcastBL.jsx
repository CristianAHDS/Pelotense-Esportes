...
/* Identidade Bundesliga */
const BL_VERMELHO = '#D20515'
const BL_GRAFITE = '#17191d'
...
const CorpoPlacar = styled.div`
  display: flex;
  align-items: stretch;
  background: linear-gradient(180deg, #22262b 0%, ${BL_GRAFITE} 100%);
...
const StatusBar = styled.div`
  ...
  .label {
    ...
    letter-spacing: 3px;
    text-transform: uppercase;
    color: ${({ $ativo }) => ($ativo ? '#ff5a67' : '#ffffff')};
  }
`
...
const FaixaGol = styled.span`
  ...
  span {
    ...
    text-transform: uppercase;
    color: #ffffff;
    filter: drop-shadow(0 0 6px rgba(210, 5, 21, 0.7));
    ...
    <Tela $compacto={compacto}>
      ...
      <BarraPlacar>
        ...
        <CorpoPlacar $penaltis={periodo === 'PÊNALTIS'}>
          {periodo === 'PÊNALTIS' ? (
            <>
              <BlocoTime $cor={estado.corCasa} $lado="casa" $gol={golAtivo?.lado === 'casa'}>
              ...
              <BlocoTime $cor={estado.corVisitante} $lado="visitante" $gol={golAtivo?.lado === 'visitante'}>
